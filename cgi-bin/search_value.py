...
def get_building_location(rows, model):
    properties = []
    log_table = {}
...
def get_complex_location(rows, model):
    properties = []
    log_table = {}
...
def get_ranked_properties(city, model, display, features):
    ...
    table = get_table_name(city=city, model=model)
    
    sql = make_sql(table, target=target, cond=cond)
    ...
    if display == 'complex':
        locations = get_complex_location(rows, model)
    else:
        locations = get_building_location(rows, model)
